...
class FirebaseAuthService implements AuthService {
  ...
  Future<AppUser> signInWithGoogle() async {
    // For web, use signInWithPopup; for mobile, use google_sign_in
    if (kIsWeb) {
    ...
    await _auth.verifyPhoneNumber(
      ...
      verificationCompleted: (fa.PhoneAuthCredential credential) async {
        ...
        if (!completer.isCompleted) {
          // When auto verified, we complete with empty verificationId to indicate no manual code needed
          completer.complete('');
        }
      },
      ...
      codeAutoRetrievalTimeout: (String verificationId) {
        // If timeout occurs before codeSent, still provide verificationId for manual entry
        if (!completer.isCompleted) {
          completer.complete(verificationId);
        }
      },
